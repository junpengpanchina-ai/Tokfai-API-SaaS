...
export async function listAdminRechargePlans(): Promise<AdminRechargePlanListItem[]> {
  const { data, error } = await supabase()
    .from("recharge_plans")
    .select(
      "id, name, amount_cents, credits, bonus_credits, stripe_price_id, enabled, visible, sort_order, badge, updated_at"
    )
...
export async function listBillingRechargePlans(): Promise<BillingRechargePlan[]> {
  const { data, error } = await supabase()
    .from("recharge_plans")
    .select(
      "id, name, amount_cents, credits, bonus_credits, enabled, visible, sort_order, badge"
    )
    .eq("visible", true)
    .order("sort_order", { ascending: true })
    .order("id", { ascending: true });
