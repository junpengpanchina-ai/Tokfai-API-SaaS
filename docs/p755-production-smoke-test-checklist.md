...
| 2.4 | 中英文切换文案一致（如启用 i18n） | `[ ]` | 未在本轮切换语言复测，非阻塞 |
...
| 4.5 | 错误密码 / 网络错误有友好提示（非 raw error） | `[ ]` | 未在本轮故意输错密码复测，非阻塞 |
...
| 6.5 | **余额不足**：友好提示 + 充值 / pricing 入口 | `[ ]` | 未复现余额不足场景，非阻塞 |
| 6.6 | **上游失败**：「模型暂时不可用或负载较高」+ 建议 gpt-5.4/5.5 | `[ ]` | 未复现上游超时/502，非阻塞 |
...
| 7.4 | **上游失败**：「图片模型暂时不可用或生成较慢」+ 失败通常不扣费说明 | `[ ]` | 未复现上游失败，非阻塞 |
...
**第二轮摘要（P756，2026-06-11）**：登录态 Dashboard 深层验收完成。Chat/Image Playground 成功路径、API Key 默认选中、Models/Usage/Credits 对账文案均通过；无效 Key 友好提示已确认。未复现项（余额不足、上游失败、错误密码、i18n 切换）共 5 项，均非阻塞。未改 billing / checkout / webhook / DMIT / PM2 / Nginx。

**P756 未复现项（非阻塞）**：2.4、4.5、6.5、6.6、7.4 — 可在后续运维中按需补测。
