-- Make credit order status the first idempotency guard for Stripe top-ups.
-- Older deployments wrote top-up ledger rows with reference_id = session.id and
-- reason = 'stripe_checkout'. Newer deployments use
-- reference_id = 'stripe_checkout:' || session.id and
-- reason = 'stripe_checkout_completed'. Treat either ledger shape as already
-- credited so one Checkout session cannot be credited twice during migration.
