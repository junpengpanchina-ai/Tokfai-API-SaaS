-- Fix Stripe Checkout top-up completion idempotency.
-- The ledger reference is the source of truth, so a paid order without a
-- matching ledger row is repaired by crediting the profile and inserting it.
